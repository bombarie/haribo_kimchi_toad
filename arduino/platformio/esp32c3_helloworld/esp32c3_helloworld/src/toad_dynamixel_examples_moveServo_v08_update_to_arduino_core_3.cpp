...
void setup()
{
  ...
  // ledcAttach is the new 3.0 API
  ledcAttach(EYE_LEFT_PIN, 5000, 8);
  ledcAttach(EYE_RIGHT_PIN, 5000, 8);
...
void updateLEDs()
{
  ...
  ledcWrite(EYE_LEFT_PIN, eye_left_val);
  ledcWrite(EYE_RIGHT_PIN, eye_right_val);
}
...
void updateSerialDebugPrint()
{
  if (millis() - prevSerialPrint > serialPrintInterval)
  {
    ...
    {    case DISCONNECTED:
      ...
      break;
    case CONNECTION_ESTABLISHED:
      Serial.print("CONNECTION_ESTABLISHED");
      break;
    case CONNECTION_LOST:
